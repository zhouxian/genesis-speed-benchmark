...
if free_drop:
    ...
    dof_pos = torch.tile(
        torch.tensor([0, 0, 0, -1.0, 0, 0, 0, 0.02, 0.02], device='cuda'),
        (n_envs, 1)
    )
    pos_action = gymtorch.unwrap_tensor(dof_pos)
...
for i in range(1000):
    gym.simulate(sim)
    gym.fetch_results(sim, True)
    gym.set_dof_position_target_tensor(sim, gymtorch.unwrap_tensor(dof_pos + torch.rand((n_envs, 9), device='cuda')*0.4-0.2))
